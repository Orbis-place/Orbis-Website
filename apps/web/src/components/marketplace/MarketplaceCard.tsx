...
export default function MarketplaceCard({ item, viewMode }: MarketplaceCardProps) {
    ...
    // Gallery View - Compact uniform cards
    if (viewMode === 'gallery') {
        return (
            <div className="bg-[#06363D] border border-[#084B54] hover:border-[#109EB1] rounded-[20px] overflow-hidden flex flex-col h-full transition-colors duration-200">
                {/* Header with icon and title */}
                <div className="flex gap-2.5 p-2.5">
                    {/* Icon - 56x56 */}
                    <div className="relative w-14 h-14 flex-shrink-0 rounded-[12px] overflow-hidden bg-[#032125]">
                        <EntityAvatar
                            src={item.image}
                            name={item.title}
                            variant="resource"
                            className="h-full w-full rounded-[12px]"
                            fallbackClassName="text-xl rounded-[12px]"
                        />
                    </div>

                    {/* Title and Author */}
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                        <h3 className="font-hebden font-semibold text-sm leading-[18px] text-[#C7F4FA] truncate">
                            {item.title}
                        </h3>
                        <span
                            className="font-hebden text-[11px] leading-[14px] text-[#C7F4FA]/50 hover:text-[#109EB1] transition-colors cursor-pointer truncate"
                            onClick={handleAuthorClick}
                        >
                            by <span className="text-[#109EB1] hover:underline">{item?.author || item.authorDisplay}</span>
                        </span>
                ...
                <div className="px-2.5 flex-1">
                    <p className="font-nunito text-[11px] leading-[14px] text-[#C7F4FA]/70 line-clamp-2">
                        {item.description}
                    </p>
                </div>
                ...
                <div className="flex flex-wrap gap-1 px-2.5 py-2">
                    {(item.categories || []).slice(0, 2).map((category, i) => (
                        <span
                            key={`cat-${i}`}
                            className="px-1.5 py-0.5 rounded-[4px] font-nunito text-[10px] leading-[12px] bg-[#109EB1]/30 text-[#C7F4FA] border border-[#109EB1]"
                        >
                            {category}
                        </span>
                    ))}
                    {(item.tags || []).slice(0, 2 - (item.categories?.length || 0)).map((tag, i) => (
                        <span
                            key={`tag-${i}`}
                            className="px-1.5 py-0.5 rounded-[4px] font-nunito text-[10px] leading-[12px] bg-[#C7F4FA]/25 text-[#C7F4FA]"
                        >
                            {tag}
                        </span>
                    ))}
                    {(item.remainingCount !== undefined && item.remainingCount > 0) && (
                        <span className="px-1 font-nunito text-[10px] leading-[12px] text-[#109EB1]">
                            +{item.remainingCount}
                        </span>
                    )}
                </div>
                ...
                {/* Footer stats */}
                <div className="flex justify-between items-center px-2.5 py-2 border-t border-[#084B54] mt-auto">
                    <div className="flex items-center gap-2">
                        <div className="flex items-center gap-1">
                            <Heart className="w-3 h-3 fill-[#C7F4FA]/50 text-[#C7F4FA]/50" />
                            <span className="font-hebden text-[10px] text-[#C7F4FA]/50">{item.likes}</span>
                        </div>
                        <div className="flex items-center gap-1">
                            <Download className="w-3 h-3 text-[#C7F4FA]/50" />
                            <span className="font-hebden text-[10px] text-[#C7F4FA]/50">{item.downloads}</span>
                        </div>
                    </div>
                    <button className="w-6 h-6 bg-[#109EB1] rounded-full flex items-center justify-center">
